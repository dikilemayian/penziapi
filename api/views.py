...
class MatchProcessor:
    ...
    def process_match_message(self, message_content, msisdn):
        ...
        # resturn string            
            ...
            elif message_content == 'PREV':
                return self.get_prev_matches(msisdn)
    ...
    def get_prev_matches(self, msisdn):
        try:
            last_queried_id_obj, _ = UpdateNext.objects.get_or_create(msisdn=msisdn)
            ...
            if last_queried_id > 0:
                prev_matches = self.potential_matches[max(0, last_queried_id - 3):last_queried_id]
                response = self.format_matches_response(prev_matches)

                last_queried_id_obj.last_queried_id = max(0, last_queried_id - 3)
                last_queried_id_obj.save()

                return response

            else:
                return "Already at the beginning of potential matches."

        except Exception as e:
            self.logger.exception("An error occurred while fetching previous potential matches.")
            return "An error occurred while fetching previous potential matches."

    def format_matches_response(self, potential_matches):
        response = ""
        for match in potential_matches:
            response += f"  {match[1]},  Aged: {match[2]}, Gender: {match[3]}, County: {match[4]}, Town: {match[5]},  MSISDN: {match[6]},  Level of Education: {match[7]}, Profession: {match[8]}, Marital Status: {match[9]}, Religion: {match[10]}, Ethnicity: {match[11]}, Description: {match[12]},"
...
class PenziMessageView(APIView):
    ...
    def get(self, request):
        try:
            # Fetch matches here using the MatchProcessor
            msisdn = request.query_params.get('msisdn')
            if not msisdn:
                return Response({'error': 'MSISDN parameter is required in the request'}, status=status.HTTP_400_BAD_REQUEST)
            
            matches = self.match_processor.get_next_matches(msisdn)
    ...
    def generate_response_message(self, message_content, msisdn):
        if message_content.startswith('penzi'):
            ...
            return self.match_processor.get_next_matches(msisdn)
        elif message_content == 'PREV':
            return self.match_processor.get_prev_matches(msisdn)
